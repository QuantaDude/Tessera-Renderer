...
typedef void (*glDebugMessageCallbackPROC)(DEBUGPROC callback, const void* userParam);
...
typedef void (*glUniform1fPROC)(GLint location, const GLfloat v0);
typedef void (*glUniform1fvPROC)(GLint location, GLsizei count, const GLfloat* value);
typedef void (*glObjectLabelPROC)(GLenum      identifier,
                                  GLuint      name,
                                  GLsizei     length,
                                  const char* label);
...
glObjectLabelPROC    glObjectLabelSRC    = NULL;
glBindBufferBasePROC glBindBufferBaseSRC = NULL;
...
#define glObjectLabel glObjectLabelSRC
#define glBindBufferBase glBindBufferBaseSRC
...
int RGL_loadGL3(RGLloadfunc proc) {
  ...
  RGL_PROC_DEF(proc, glUniform1fv);
  RGL_PROC_DEF(proc, glObjectLabel);
  RGL_PROC_DEF(proc, glBindBufferBase);

  if (glShaderSourceSRC == NULL || glCreateShaderSRC == NULL || glCompileShaderSRC == NULL ||
      glCreateProgramSRC == NULL || glAttachShaderSRC == NULL || glBindAttribLocationSRC == NULL ||
      glLinkProgramSRC == NULL || glBindBufferSRC == NULL || glBufferDataSRC == NULL ||
      glEnableVertexAttribArraySRC == NULL || glVertexAttribPointerSRC == NULL ||
      glDisableVertexAttribArraySRC == NULL || glDeleteBuffersSRC == NULL ||
      glDeleteVertexArraysSRC == NULL || glUseProgramSRC == NULL || glDetachShaderSRC == NULL ||
      glDeleteShaderSRC == NULL || glDeleteProgramSRC == NULL || glBufferSubDataSRC == NULL ||
      glGetShaderivSRC == NULL || glGetShaderInfoLogSRC == NULL || glGetProgramivSRC == NULL ||
      glGetProgramInfoLogSRC == NULL || glGenVertexArraysSRC == NULL || glGenBuffersSRC == NULL ||
      glBindVertexArraySRC == NULL || glGetUniformLocationSRC == NULL ||
      glUniformMatrix4fvSRC == NULL || glUniform1iSRC == NULL || glUniform4fvSRC == NULL ||
      glUniform1fSRC == NULL || glUniform1fvSRC == NULL || glObjectLabelSRC == NULL ||
      glBindBufferBaseSRC == NULL || glClearSRC == NULL || glClearColorSRC == NULL ||
      glViewportSRC == NULL)
    return 1;
