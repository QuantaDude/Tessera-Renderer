...
// - When calling Init with 'install_callbacks=true': ImGui_ImplRgfw_InstallCallbacks() is called.
// RGFW callbacks will be installed for you. They will chain-call user's previously installed
// callbacks, if any.
// - When calling Init with 'install_callbacks=false': RGFW callbacks won't be installed. You will
// need to call individual function yourself from your own RGFW callbacks.
...
IMGUI_IMPL_API void ImGui_ImplRgfw_WindowFocusCallback(RGFW_window* window,
                                                       u8           inFocus); // Since 1.84
IMGUI_IMPL_API void
ImGui_ImplRgfw_CursorEnterCallback(RGFW_window* window, i32 x, i32 y, u8 status); // Since 1.84
IMGUI_IMPL_API void ImGui_ImplRgfw_CursorPosCallback(
    RGFW_window* window, i32 x, i32 y, float vecX, float vecY); // Since 1.87
IMGUI_IMPL_API void ImGui_ImplRgfw_MouseButtonCallback(RGFW_window* window, u8 button, u8 pressed);
IMGUI_IMPL_API void
ImGui_ImplRgfw_MouseScrollCallback(RGFW_window* window, float xoffset, float yoffset);
IMGUI_IMPL_API void
ImGui_ImplRgfw_KeyCallback(RGFW_window* window, u8 keycode, u8 modState, u8 repeat, u8 pressed);
...
struct ImGui_ImplRgfw_Data {
  ...
  // Chain RGFW callbacks: our callbacks will call the user's previously installed callbacks, if
  // any.
...
static ImGui_ImplRgfw_Data* ImGui_ImplRgfw_GetBackendData() {
  return ImGui::GetCurrentContext() ? (ImGui_ImplRgfw_Data*)ImGui::GetIO().BackendPlatformUserData
                                    : nullptr;
}
...
static ImGuiKey ImGui_ImplRgfw_KeyToImGuiKey(int key) {
  switch (key) {
    ...
    case RGFW_return:
      return ImGuiKey_Enter; // Same as RGFW_enter
    case RGFW_delete:
    ...
    case RGFW_F24:
      return ImGuiKey_F24;
    case RGFW_F25:
      return ImGuiKey_None; // No ImGuiKey_F25
    case RGFW_capsLock:
    ...
    case RGFW_scrollLock:
      return ImGuiKey_ScrollLock;
    case RGFW_printScreen:
      return ImGuiKey_PrintScreen;
    case RGFW_pause:
      return ImGuiKey_Pause;
    case RGFW_world1:
      return ImGuiKey_None; // Not supported
    case RGFW_world2:
      return ImGuiKey_None; // Not supported
    default:
...
void ImGui_ImplRgfw_KeyCallback(
    RGFW_window* window, u8 key, u8 modState, RGFW_bool repeat, RGFW_bool pressed) {
  ImGui_ImplRgfw_Data* bd = ImGui_ImplRgfw_GetBackendData();
  ...
  // io.SetKeyEventNativeData(imgui_key, keyChar, key); // To support legacy indexing (<1.87 user
  // code)
}
...
EM_JS(void, ImGui_ImplRgfw_EmscriptenOpenURL, (char const* url), {
...
static bool
ImGui_ImplRgfw_Init(RGFW_window* window, bool install_callbacks, RgfwClientApi client_api) {
  ImGuiIO& io = ImGui::GetIO();
  IMGUI_CHECKVERSION();
...
#    ifdef __EMSCRIPTEN__
  ...
  // ImGui_ImplRgfw_EmscriptenOpenURL(url); return true; };
#    endif
  // Chain RGFW callbacks: our callbacks will call the user's previously installed callbacks, if
  // any.
    ...
    if (is_window_focused) {
      ...
      if (io.WantSetMousePos)
        RGFW_window_moveMouse(
            window, static_cast<i32>(io.MousePos.x), static_cast<i32>(io.MousePos.y));

      // (Optional) Fallback to provide mouse position when focused
      // (ImGui_ImplRgfw_CursorPosCallback already provides this when hovered or captured)
...
static void ImGui_ImplRgfw_UpdateMouseCursor() {
  ...
  // (those braces are here to reduce diff with multi-viewports support in 'docking' branch)
  {
    ...
    if (imgui_cursor == ImGuiMouseCursor_None || io.MouseDrawCursor) {
      ...
      // FIXME-PLATFORM: Unfocused windows seems to fail changing the mouse cursor with RGFW 3.2,
      // but 3.3 works here.
...
void ImGui_ImplRgfw_NewFrame() {
  ...
  IM_ASSERT(bd != nullptr &&
            "Context or backend not initialized! Did you call ImGui_ImplRgfw_InitForXXX()?");
  ...
  double current_time =
      duration_cast<milliseconds>(steady_clock::now().time_since_epoch()).count() / 1000.0f;
  if (current_time <= bd->Time) {
    current_time = bd->Time + 0.000001;
  }
  io.DeltaTime = bd->Time == 0.0 ? static_cast<float>(1.0f / 60.0f)
                                 : static_cast<float>(current_time - bd->Time);
